<template>
  <div class="signupdiv">
    <transition name="errorMsg" mode="out-in">
        <p v-if="showError" class="errorMsg top-5">This Email has already been used!</p>
    </transition>
    <h1 class="font-bold text-3xl mb-5"><span class="text-green-600">SIGN</span> UP</h1>
    ...
    <form @submit.prevent="signUp" class="w-11/12 md:w-5/12 flex flex-col items-center justify-center">
        ...
        <button v-if="loading" disabled type="button" class="py-2.5 px-3 mr-2 text-sm font-bold text-green-600 bg-white rounded-lg border border-gray-200 shadow focus:z-10 focus:ring-2 focus:ring-green-700 focus:text-green-700 inline-flex items-center">
